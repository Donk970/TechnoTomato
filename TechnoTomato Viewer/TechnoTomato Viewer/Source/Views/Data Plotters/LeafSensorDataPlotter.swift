...
class LeafSensorDataPlotter: TechnoTomatoDataPlotter {
    ...
    func drawDataPoints(for plant: TechnoTomatoPlant, in rect: CGRect) {
        ...
        let hs: CGFloat = W/C 
        let vs: CGFloat = plant.sensorScale(for: rect)
        let min: CGFloat = plant.minDisplayedSensorReading
        ...
        for entry in plant.entries {
            ...
            let dy: CGFloat = H - ((entry.leafValue - min) * vs)
